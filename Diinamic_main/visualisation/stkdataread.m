function [stack_info,stack] = stkdataread(pn,img_last,fn)
% function [stack_info,stack] = stkdataread(pn,img_last,fn)
% pn: file name
% fn: folder
% stack: image/s
...
while (ifd_pos ~= 0)
    ...
    if isfield( TIF, 'MM_stack' )
       ...
       % end
        ...
        stack_info.dt=0; %%%%%%%%%%%%%%%%%%%%%%OJO
        if exist('waitbar_handle', 'var')
           waitbar( img_read/img_last, waitbar_handle);
        end
    end
end
...
%waitbar_handle = waitbar(0,'Converting data...','Name',['Reading ' filename]);

%for k=1:stack_info.frames
   % stack_data(:,:,k)=double(stack(k).data);
             %  if exist('waitbar_handle', 'var')
             %  waitbar( k/stack_info.frames, waitbar_handle);
           %end

    %stack(k).data=double(stack(k).data);
%end;

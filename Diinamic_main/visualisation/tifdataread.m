function [stack_info,stack] = tifdataread(filename,img_last)
% function [stack_info,stack] = tifdataread(filename,img_last)
% stack: image/s
% stack_info: structure with .x, .y, .frames, .dt
...
%
% from stk data reader (MatLab forum)
% Marianne Renner 09/09   SPTrack_v4
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while (ifd_pos ~= 0)
    ...
    if code258==1 %tif not supported
       [stack_info,stack]=readotra(filename);
       return
    end
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        CreationTime = zeros(1,TIF.MM_stackCnt);
        CreationTime_idx = [4:6:6*TIF.MM_stackCnt];
        for k=1:TIF.MM_stackCnt
            CreationTime(k)=TIF.MM_stack(CreationTime_idx(k));
        end;
        stack_info.frames = TIF.MM_stackCnt;
        stack_info.dt = round(median(diff(CreationTime)));
        break;
    else
...
stack_info.depth = stack(1).bits;
stack_info.x = stack(1).width;
stack_info.y = stack(1).height;

%stack_data = zeros(stack_info.y,stack_info.x,stack_info.frames);
%for k=1:stack_info.frames
%    stack_data(:,:,k)=double(stack(k).data);
%end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [stack_info,stack_data]=readotra(filename)

stack_data=imread(filename);
info=imfinfo(filename);
stack_info.x=info.Width;
stack_info.y=info.Height;
stack_info.frames = 1;
stack_info.dt=0; %%%%%%%%%%%%%%%%%%%%%%OJO

function newpk=buildTrackscorrdet(pk, Dmax, till,szpx,blinkdist,blinktime)
% function reconnectedfile=buildTrackscorrdet(pk,szpx,till)
%
...
disp('Correction multiple detections...')
waitbarhandle = waitbar( 0,'Please wait...','Name',['Finding positions of the same molecule']) ;

%Dmax=(blinkdist/1000)^2/(till/1000*4);
blinktime=100;

T = 3;
%T = blinktime+5;
T_off = 1;  %blinking % VOIR!!!!!!!!!!!!!!
%T_off = blinktime;  %blinking % VOIR!!!!!!!!!!!!!!
%Dmax = 0.001;  
Boule_free = 1;     
Nb_combi = 3;             % competitors
Poids_melange_diff = 1;
%blinktime = 300;
%blinkdist = 1;  
limdist = NaN;
mintrace = 2;
...
assignin('base', 'Poids_melange_diff', Poids_melange_diff)
    ...
    for n=1:numel(trackList)
        if exist('waitbarhandle')
            waitbar(n/numel(trackList),waitbarhandle,['Molecule # ',num2str(n)]);
        end
        ...
        if length>1
            for jj=1:length
               ...
               indexframe=find(pk(:,1)==frame);
                ...
                trc=[trc; count trackList{n}(jj,3) trackList{n}(jj,2) trackList{n}(jj,1) ]; % 
            end
            count=count+1;
        end
...
if isempty(reconnectedfile)==0 
        ...
        %VOIR!!!!!!!!!!!!
        pkaux = [pk zeros(size(pk,1),1)];
        
        for i=1:size(reconnectedfile,1)
            
            index=find(pkaux(:,2)==reconnectedfile(i,4));
            if isempty(index)==0
                index2=find(pkaux(index,3)==reconnectedfile(i,3));
                 if isempty(index2)==0
        ...
        pkmock=pkaux;
        if size(pkaux,2)<11
           for addcol=size(pkaux,2)+1:11
               pkmock=[pkmock zeros(size(pkaux,1),1)];
           end
        end
        
        for i=1:max(pkaux(:,size(pkaux,2)))+1
            indexi=find(pkaux(:,size(pkaux,2))==i-1); %ordre
            if isempty(indexi)==0
                if i-1==0 %not tracked
                    newpk=[newpk; pkmock(indexi,1:size(pkmock,2))];
                else
                    if size(newpk,1)<1 && size(newpk,2)==size(pkmock,2)
                    ...
                    newpk=[newpk; pkmock(indexi(1),1) mean(pkmock(indexi,2)) mean(pkmock(indexi,3))  mean(pkmock(indexi,4)) mean(pkmock(indexi,5)) mean(pkmock(indexi,6)) mean(pkmock(indexi,7))...
                    mean(pkmock(indexi,8)) mean(pkmock(indexi,9)) mean(pkmock(indexi,10)) mean(pkmock(indexi,11))];
                    end
                end
            end
        end
...
if plotcontrol
       ...
       for i=1:max(trc(:,1))
           index=find(trc(:,1)==i);
           if isempty(index)==0
               plot(trc(index,4),trc(index,3),'-g');
           end
       end
           
end

clear pkaux trc reconnectedfile
%end %function

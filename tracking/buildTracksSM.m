...
prompt = {'Averaging time window (frames):','Blinking probability (0 to 1):', 'Max diffusion coefficient (um^2/s):',...
    'Research diameter (pixels):','Max competitors :','Weight max local diffusion (0 to 1):',...
    'Pixel size (nm):','Time between frames (ms):'};
num_lines = 1;
dlg_title = 'MTT traces';
def = {'3','1','1','1','3','1','160','30'}; % default values
...
T = str2num(answer{1});%#ok<ST2NM>
T_off = str2num(answer{2})*-1;%#ok<ST2NM>   %blinking
Dmax = str2num(answer{3});%#ok<ST2NM>   
Boule_free = str2num(answer{4}); %#ok<ST2NM>     
Nb_combi = str2num(answer{5});%#ok<ST2NM>              % competitors
Poids_melange_diff = str2num(answer{6});%#ok<ST2NM>
szpx = str2num(answer{7});%#ok<ST2NM>
till = str2num(answer{8});%#ok<ST2NM>
...
for cont=1:size(listafiles,2)
    ...
    sig_free = sqrt(Dmax/(szpx/1000)^2 * 4*till/1000);
   ...
   % peaks = load(file);
    ...
    % clean pk data
   % pk = cleanpk (peaks, opt, 2); % size and intensity
   ...
   % pk=peaks; %!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for t = 1:max(pk(:,1))  
        ...
        mise_a_jour_tab(new_nb_traj,tab_param,T_off, t_red,Nb_STK,tab_moy,tab_var,sig_free) %nested by CPR; mod by MR

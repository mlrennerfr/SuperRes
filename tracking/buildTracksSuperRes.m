function buildTracksSuperRes(handles)
% function buildTracksSuperRes(handles)
%
% builds trajectories using data of .pk files from DetectSM and
% modified tracking algorithms of MTT package
% output: .trc file (initial tracking!)
%         .con.trc (reconnected)
%
% Marianne Renner june 2016: Modified from Builtracks_v2 (Marianne Renner dec 2012)
% Marianne Renner july 2017: include intensity values in trc file
% Marianne Renner feb 2025 : integrated in SuperRes_v4_app
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
wn = 7; %MOCK
t_red = handles.T-handles.T_off ; 

%t == T-T_off dans les tab_x reduits
%sig_free =sqrt(Dmax/(szpx/1000)^2 * 4*till/1000); % modif FQ
%seuil_detec_1vue = chi2inv(1-10^errorRate,1);

assignin('base', 't_red', t_red);
assignin('base', 'T', handles.T);
assignin('base', 'T_off', handles.T_off);
% assignin('base', 'sig_free', sig_free); % modif FQ
assignin('base', 'Boule_free', handles.Boule_free);
assignin('base', 'Nb_combi', handles.Nb_combi);
%assignin('base', 'Poids_melange_aplha', Poids_melange_aplha)
assignin('base', 'Poids_melange_diff', handles.Poids_melange_diff)
...
currentdir=cd;
path=[directory_name,'/pk'];
if isdir([directory_name,'/trc']); else mkdir([directory_name,'/trc']);end
cd(path);
...
disp('  ');
disp('Tracking and reconnecting trajectories...');
...
for cont=1:size(listafiles,2)
    ...
    pk = [];  %#ok<NASGU>
    % file names
    file=[st{listafiles(cont)}] %#ok<NOPRT>
    [namefile,rem]=strtok(file,'.');   %#ok<NASGU>
    
    namefileDot = regexprep(namefile, ',', '.');
    %PositionTau = strfind(namefileDot, 'mm-(t=');
    %PositionMS = strfind(namefileDot, 'ms)_');

    %till = str2num(namefileDot(PositionTau + numel('mm-(t=') : PositionMS - 1));
    sig_free = sqrt(handles.Dmax/(handles.szpx/1000)^2 * 4*handles.till/1000);
    assignin('base', 'sig_free', sig_free);

    % detection data    
   % peaks = load(file);
    pk = load(file);
    waitbarhandle = waitbar( 0,'Please wait...','Name',['Initial tracking in ',file]) ;
    
    %initialize data
    Nb_STK = max(pk(:,1));
    assignin('base', 'Nb_STK', Nb_STK)
    ctrsN = zeros(Nb_STK,1);    
    for i = 1:max(pk,1);
        indexfr = find(pk(:,1)==i);
        ctrsN = [ctrsN; size(indexfr,1)]; %#ok<AGROW>
    end
    data.ctrsX = pk(:,2);
    data.ctrsY=pk(:,3);
    data.signal=pk(:,5);
    ...
    for t = 1:max(pk(:,1))  
        ...
        if exist('waitbarhandle')
            waitbar(t/max(pk(:,1)),waitbarhandle,['Frame # ',num2str(t)]);
        end
        ...
        if t == 1     
            ...
            par_per_frame(t) = sum(ind_valid); %#ok<AGROW>
            lest = [(1:par_per_frame(t))' data.ctrsY(ind_valid) data.ctrsX(ind_valid) data.signal(ind_valid)*sqrt(pi).*data.radius(ind_valid)...
             ...
             tab_param(:,2+7*(t_red-1):1+7*(t_red)) = [ones(par_per_frame(t),1),  lest(:,[2,3,4,6]), zeros(par_per_frame(t),1), Nb_STK*ones(par_per_frame(t),1)];
             tab_param(:,2+7*(t_red)) = 2*ones(par_per_frame(t),1) ;
             tab_var(:,1)   = (1:par_per_frame(t))' ;
             tab_var(:,2+7*(t_red-1):1+7*(t_red)) = [ones(par_per_frame(t),1), zeros(par_per_frame(t),4), lest(:,5), Nb_STK*ones(par_per_frame(t),1)];
             ...
             for track = tab_param(:,1)'
                 if tab_param(track,1+7*(t_red))>0
                     trackList{trackID(track)}(1,:) = [tab_param(track,7*(t_red-1)+[4 3 2]) trackID(track) tab_param(track,7*(t_red-1)+5)  tab_var(track,7*(t_red-1)+[7 3 5])];
                 end %if
             end %for  
             continue
        end
        ...
        par_per_frame(t) = sum(ind_valid); %#ok<AGROW>
        lest = [(1:sum(ind_valid))' data.ctrsY(ind_valid) data.ctrsX(ind_valid) data.signal(ind_valid)*sqrt(pi).*data.radius(ind_valid) data.noise(ind_valid).^2 data.radius(ind_valid) ones(par_per_frame(t),1)];

        nb_traj_active = 0 ;
        nb_traj_blink = 0 ;
        isClosed = (tab_param(:,7*(t_red-1)+8) == handles.T_off);
        ...
        if ~isempty(tab_param)
            ...
            [mock, part_ordre_blk] = sort(-tab_param(:,7*(t_red-1)+8)) ;
            
            for traj = part_ordre_blk'
                %% reconnection test
                if (tab_param(traj, 7*(t_red-1)+8) > handles.T_off)
                ...
                %% update tables
                if (part>0)
                    tab_param(traj, 7*(t_red)+[3 4 5 6]) = lest(part, [2 3 4 6]);
                    ...
                    if (tab_param(traj, 7*(t_red-1)+8) > 0) %trajectory was not OFF
                        ...
                        %[mock, flag_full_alpha] = rapport_detection(traj, t_red-1, lest, part, wn) ;
                        
                        flag_full_alpha=1; %full ON
                        ...
                        if (flag_full_alpha==1) %full ON
                            ...
                            tab_param(traj, 7*(t_red)+8) =   tab_param(traj, 7*(t_red)+8) - mod(tab_param(traj, 7*(t_red)+8), Nb_STK);
                        end%if
                    else
                    ...
                    tab_param(traj, 7*(t_red)+[3 4 5 6]) =  [tab_param(traj, 7*(t_red-1)+[3 4]) 0 0];
                    ...
                    if (t>3)
                        if (tab_param(traj, 7*(t_red-2)+8) == 0)
                            tab_param(traj, 7*(t_red)+8) = handles.T_off ;
                        end %if
                    end %if        
        ...
        %check on particles not assigned to an active trajectory and in case initiate a new one
        
        nb_non_aff_detect = false(par_per_frame(t),1);
        %nb_non_aff_detect = true(par_per_frame(t),1);   
        ...
        tab_param_new = [(1:new_traj)'+nb_traj_avant_new, zeros(new_traj,7*(t_red)), [ones(new_traj,1)*t, lest(nb_non_aff_detect, [2 3 4 6]), zeros(new_traj,1) ones(new_traj,1)*Nb_STK]]; %by CPR
        tab_param = [tab_param; tab_param_new]; %#ok<AGROW>
        tab_var = [tab_var; zeros(new_traj,7*(t_red+1)+1)]; %#ok<AGROW>
        tab_moy = [tab_moy; zeros(new_traj,7*(t_red+1)+1)]; %#ok<AGROW>
        tab_var((1:new_traj)+nb_traj_avant_new, 7*(t_red)+7) = lest(nb_non_aff_detect, 5) ; %sig2_b
   
        %% shift tables one step to the left
        new_nb_traj = size(tab_param, 1) ;
        tab_param = [tab_param(:,1),  tab_param(:,2+7*(1):1+7*(t_red+1)),   (t+1)*ones(new_nb_traj,1), zeros(new_nb_traj,6)] ;%correction arnauld
        tab_var = [tab_var(:,1), tab_var(:,2+7*(1):1+7*(t_red+1)),  (t+1)*ones(new_nb_traj,1), zeros(new_nb_traj,6)] ;
        tab_moy = [tab_moy(:,1), tab_moy(:,2+7*(1):1+7*(t_red+1)), (t+1)*ones(new_nb_traj,1), zeros(new_nb_traj,6)] ;

        %init_tab((1+nb_traj_avant_new):new_nb_traj) ; %nested by CPR;
        tab_traj = (1+nb_traj_avant_new):new_nb_traj ;
        new = 1 ;

        mise_a_jour_tab(new_nb_traj,tab_param,handles.T_off, t_red,Nb_STK,tab_moy,tab_var,sig_free) %nested by CPR; mod by MR

        %% update tracks
        trackList = [trackList cell(1,new_traj)] ;       %#ok<AGROW>

        for track = tab_param(:,1)'

            if tab_param(track,1+7*(t_red))>0
                trackList{trackID(track)}(end+1,:) = [tab_param(track,7*(t_red-1)+[4 3 2]) trackID(track) tab_param(track,7*(t_red-1)+5)  tab_var(track,7*(t_red-1)+[7 3 5])];
            end %if

        end %for

        %waitbar(t/Nb_STK,hProgressbar,['Processing Frame: ' num2str(t) '/' num2str(Nb_STK)],'Color',get(0,'defaultUicontrolBackgroundColor'));
  ...
  %  tic;
    ...
    if exist('waitbarhandle')
       waitbar(1,waitbarhandle,['Please wait...']);
    end
    %aux=[];
 
    for n=1:numel(trackList)
       length=size(trackList{n},1);
        if length>3 %!!!!!!!!!!!!!!!!!!!!! reduces spurious trajectories?
            %intensity
                %data.ctrsX = pk(:,2);    data.ctrsY=pk(:,3);    data.signal=pk(:,5);
                
               
            for jj=1:length
                frame=trackList{n}(jj,3);
               indexframe=find(pk(:,1)==frame);
                index=find(pk(indexframe,2)==trackList{n}(jj,1)); %att transposed!!
               % index=find(pk(:,2)==trackList{n}(jj,1));  %att transposed!!
                if size(index,1)>1
                    index2=find(pk(indexframe(index),3)==trackList{n}(jj,2));
                    %index2=find(pk(index,3)==trackList{n}(jj,2));
                    nroorder=indexframe(index(index2));
                   % nroorder=index(index2);
                else
                    nroorder=indexframe(index);
                  %  nroorder=index;
                end
                
                % transposed !
                trc=[trc; count trackList{n}(jj,3) trackList{n}(jj,1)+1 trackList{n}(jj,2)+1  pk(nroorder,5)]; %with intensity in the 5th column
               % trc=[trc; count trackList{n}(jj,3) trackList{n}(jj,2)+1 trackList{n}(jj,1)+1  pk(nroorder,5)]; %with intensity in the 5th column
                 %if count<10
                 %    aux=[aux; count pk(nroorder,:)];
                 %end
            end
                
              %     trc=[trc; count*ones(length,1) trackList{n}(:,3) trackList{n}(:,2)+1 trackList{n}(:,1)+1 zeros(length,2)];
 ...
 %   elapsedTime = toc
  ...
  cd(currentdir)
    ...
    save(['trc/',namefile,'.trc'],'trc','-ascii');
   % save(['trc/',namefile,'.pk.trc'],'aux','-ascii');
    
    %save ([namefile,'_pk.tracked.mat'],'data', '-mat')
    clear data trackList 
    close(waitbarhandle);
    
    %reconnection
    
    blinktime=handles.blinktime;
    blinkdist=handles.blinkdist;
    limdist=handles.limdist;
    mintrace=handles.mintrace;
    reconnectedfile=trc;
...
%function init_tab(new_traj)(new_traj)
...
function mise_a_jour_tab(new_nb_traj,tab_param,T_off,t_red,Nb_STK,tab_moy,tab_var,sig_free)
...
for iTraj=1:new_nb_traj
            ...
            if (tab_param(iTraj, 7*(t_red-1)+8)>T_off) %#ok<ALIGN>
                ...
                [moy, sig_alpha] = calcul_reference(5,iTraj,t_red,tab_param,Nb_STK, T,tab_moy,tab_var) ;
                alpha_moy = real(moy) ;
                alpha_max = imag(moy) ;
                ...
                if (LV_gauss > LV_uni)
                 ...
                 %% r
                [moy, sig] = calcul_reference(6,iTraj,t_red,tab_param,Nb_STK,T,tab_moy,tab_var) ;
                tab_moy(iTraj, 7*(t_red-1)+6) = moy ;
                tab_var(iTraj, 7*(t_red-1)+6) = sig ;
                
                %% i,j
                [mock, sig_i] = calcul_reference(3,iTraj,t_red,tab_param,Nb_STK,T,tab_moy,tab_var) ;
                [mock, sig_j] = calcul_reference(4,iTraj,t_red,tab_param,Nb_STK,T,tab_moy,tab_var) ;
...
function [param_ref, sig_param] = calcul_reference(param,iTraj,t_red,tab_param,Nb_STK,T,tab_moy,tab_var)

%% determine la reference (parametre moyen) et ecart type du param
%% determine sur la derniere zone de non blinket limite a une longueur T
...
%---------------------------------------------------------MR 2016
if isempty(sigij_free_blink(1:nb_traj, t))==1
    boules = Boule_free;
else
...
for p = liste_part'
 ic = lest(p,2) ;
  ...
  jcm = max(jc - boules, 0) ; 
  jcM = jc + boules ;
  traj_boule = traj_boule |  ((tab_param(:, 7*t+3) < icM) & (tab_param(:, 7*t+3) > icm) &  (tab_param(:, 7*t+4) < jcM) & (tab_param(:, 7*t+4) > jcm)) ;
...
if isempty(sigij_free_blink(traj, t))==1 %MR 2016
    boule=Boule_free;
end
...
part_boule =(lest(:,2) < icM) & (lest(:,2) > icm) &  (lest(:,3) < jcM) & (lest(:,3) > jcm) ;
...
if (traj<=0)%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%NO
  ...
  dj=[];
  for kk=1:wn
       auxdi = kk+Pi-floor(wn/2) ;
      if auxdi<size(im_t,1)+1 & auxdi>0
          di=[di, auxdi];
      end
      auxdj = kk+Pj-floor(wn/2) ;
      if dj<size(im_t,2)+1 & auxdj>0
          dj=[dj, auxdj];
      end
  end  
...
%-------------------------------------- MR2016
if isempty(sig_free)==1
    sig_free_blk = ones(nb_traj, 1) ;
else
...
% Elle ne depend ni de la position, ni de l'intensite.
  ...
  for p=2:nb_perms
    ...
    vrais_tmp =  vrais_config_reconnex(vec_traj, vec_part_perm, t,   lest, wn);%, T) ;

%% version modifiee pour compat octave
%% pb nombre args dans function fread

function [stack, stack_cnt] = tiffread(filename, img_first, img_last)
% function [stack, stack_cnt] = tiffread(filename, img_first, img_last)
% tiffread, version 2
%
...
%
% at return, nbimages contains the number of images in stack, modif AS
% and S is a vector
...
byte_order = setstr(fread(TIF.file, 2, 'uchar')); %#ok
%byte_order = fread(TIF.file, 2, 'uchar','*char');
if ( strcmp(byte_order', 'II') )
    TIF.BOS = 'l';      % "ieee-le" ;                         %normal PC format
...
tiff_id = fread(TIF.file,1,'uint16', 0,  TIF.BOS);   %% compat octave
if (tiff_id ~= 42)
    error('This is not a TIFF file (missing 42).');
end

%----- read the byte offset for the first image file directory (IFD)
ifd_pos = fread(TIF.file,1,'uint32', 0, TIF.BOS);    %% compat octave
...
while (ifd_pos ~= 0)
    ...
    num_entries = fread(TIF.file,1,'uint16', 0, TIF.BOS);    %% compat octave
    ...
    for i = 1:num_entries 
        ...
        % read entry tag
        TIF.entry_tag = fread(TIF.file, 1, 'uint16', 0, TIF.BOS);    %% compat octave
        entry = readIFDentry;
        ...
        switch TIF.entry_tag
            ...
            case 339
                ...
                if ( TIF.SampleFormat > 2 )
                    error('unsuported sample format = %i', TIF.SampleFormat);
                end
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        for ii = img_first:img_last
            ...
            if exist('waitbar_handle', 'var')
                ...
                waitbar( img_read/(img_last-img_first+1), waitbar_handle);                 
            end
...
bytes = fread( TIF.file, StripLength, typecode, 0, TIF.BOS );   %% compat octave

if ( length(bytes) ~= StripLength )  %#ok
...
global TIF;
entry.typecode = fread(TIF.file, 1, 'uint16', 0, TIF.BOS);   %% compat octave
entry.cnt      = fread(TIF.file, 1, 'uint32', 0, TIF.BOS);   %% compat octave
...
if entry.nbbytes * entry.cnt > 4
    ...
    offset = fread(TIF.file, 1, 'uint32', 0, TIF.BOS);   %% compat octave
    %disp(strcat('offset = ', num2str(offset)));
    fseek(TIF.file, offset, -1);
end

if TIF.entry_tag == 33629   %special metamorph 'rationals'
    entry.val = fread(TIF.file, 6*entry.cnt, entry.typechar, 0, TIF.BOS);   %% compat octave
else
    if entry.typecode == 5
        entry.val = fread(TIF.file, 2*entry.cnt, entry.typechar, 0, TIF.BOS);   %% compat octave
    else
        entry.val = fread(TIF.file, entry.cnt, entry.typechar, 0, TIF.BOS);   %% compat octave
    end
end
